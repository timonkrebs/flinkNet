...
/// collector.</para>
///
/// <para>Warning: If you subclass Tuple3, then be sure to either
/// <list type="bullet">
...
[Public]
public class Tuple3<T0, T1, T2> : Tuple
{
    ...
    /// <summary>
    /// Creates a string representation of the tuple in the form (f0, f1, f2), where the
    /// individual fields are the value returned by calling <see cref="object.ToString"/> on that
    /// field.
    /// </summary>
    public override string ToString() =>
        "(" + ArrayAwareToString(F0) + "," + ArrayAwareToString(F1) + "," + ArrayAwareToString(F2) + ")";
    ...
    public override bool Equals(object? obj)
    {
        ...
        if (obj is not Tuple3<T0, T1, T2> tuple)
        {
            return false;
        }
    ...
    public override Tuple3<T0, T1, T2> Copy() => new(F0, F1, F2);
}

/// <summary>
/// Factory for <see cref="Tuple3{T0, T1, T2}"/> allowing the compiler to infer the generic type
/// arguments implicitly. For example: <c>Tuple3.Of(...)</c> instead of
/// <c>new Tuple3&lt;...&gt;(...)</c>.
/// </summary>
[Public]
public static class Tuple3
{
    ...
    /// </summary>
    public static Tuple3<T0, T1, T2> Of<T0, T1, T2>(T0? f0, T1? f1, T2? f2) => new(f0, f1, f2);
}
