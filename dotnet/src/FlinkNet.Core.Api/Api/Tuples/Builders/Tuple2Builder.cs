...
/// <summary>
/// A builder class for <see cref="Tuple2{T0, T1}"/>.
/// </summary>
...
{
    private readonly List<Tuple2<T0, T1>> _tuples = [];

    public Tuple2Builder<T0, T1> Add(T0? f0, T1? f1)
    {
        _tuples.Add(new Tuple2<T0, T1>(f0, f1));
        return this;
    }

    public Tuple2<T0, T1>[] Build() => _tuples.ToArray();
}
