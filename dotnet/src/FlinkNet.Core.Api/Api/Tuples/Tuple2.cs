...
/// collector.</para>
///
/// <para>Warning: If you subclass Tuple2, then be sure to either
/// <list type="bullet">
...
[Public]
public class Tuple2<T0, T1> : Tuple
{
    ...
    /// <summary>
    /// Creates a string representation of the tuple in the form (f0, f1), where the
    /// individual fields are the value returned by calling <see cref="object.ToString"/> on that
    /// field.
    /// </summary>
    public override string ToString() =>
        "(" + ArrayAwareToString(F0) + "," + ArrayAwareToString(F1) + ")";
    ...
    public override bool Equals(object? obj)
    {
        ...
        if (obj is not Tuple2<T0, T1> tuple)
        {
            return false;
        }
    ...
    public override Tuple2<T0, T1> Copy() => new(F0, F1);
}

/// <summary>
/// Factory for <see cref="Tuple2{T0, T1}"/> allowing the compiler to infer the generic type
/// arguments implicitly. For example: <c>Tuple2.Of(...)</c> instead of
/// <c>new Tuple2&lt;...&gt;(...)</c>.
/// </summary>
[Public]
public static class Tuple2
{
    ...
    /// </summary>
    public static Tuple2<T0, T1> Of<T0, T1>(T0? f0, T1? f1) => new(f0, f1);
}
