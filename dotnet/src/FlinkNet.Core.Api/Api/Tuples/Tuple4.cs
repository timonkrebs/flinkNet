...
/// collector.</para>
///
/// <para>Warning: If you subclass Tuple4, then be sure to either
/// <list type="bullet">
...
[Public]
public class Tuple4<T0, T1, T2, T3> : Tuple
{
    ...
    public override string ToString() =>
        "(" + ArrayAwareToString(F0) + "," + ArrayAwareToString(F1) + "," + ArrayAwareToString(F2) + "," + ArrayAwareToString(F3) + ")";
    ...
    public override bool Equals(object? obj)
    {
        ...
        if (obj is not Tuple4<T0, T1, T2, T3> tuple)
        {
            return false;
        }
    ...
    public override Tuple4<T0, T1, T2, T3> Copy() => new(F0, F1, F2, F3);
}

/// <summary>
/// Factory for <see cref="Tuple4{T0, T1, T2, T3}"/> allowing the compiler to infer the generic type
/// arguments implicitly. For example: <c>Tuple4.Of(...)</c> instead of
/// <c>new Tuple4&lt;...&gt;(...)</c>.
/// </summary>
[Public]
public static class Tuple4
{
    ...
    /// </summary>
    public static Tuple4<T0, T1, T2, T3> Of<T0, T1, T2, T3>(T0? f0, T1? f1, T2? f2, T3? f3) => new(f0, f1, f2, f3);
}
