...
/// <summary>
/// <see cref="IState"/> interface for aggregating state, based on an
/// <see cref="FlinkNet.Api.Common.Functions.IAggregateFunction{TIn, TAcc, TOut}"/>. Elements that
/// are added to this type of state will be eagerly pre-aggregated using a given
/// <c>AggregateFunction</c>.
/// <para>The state holds internally always the accumulator type of the <c>AggregateFunction</c>.
/// When accessing the result of the state, the function's
/// <see cref="FlinkNet.Api.Common.Functions.IAggregateFunction{TIn, TAcc, TOut}.GetResult"/>
/// method.</para>
...
[Experimental]
public interface IAggregatingState<TIn, TOut> : IMergingState<TIn, TOut, TOut>
{
}
