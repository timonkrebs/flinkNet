...
/// value, by taking always two elements and combining them into one. Reduce functions may be used
/// on entire data sets, or on grouped data sets. In the latter case, each group is reduced
/// individually.
///
/// <para>For a reduce functions that work on an entire group at the same time (such as the
/// MapReduce/Hadoop-style reduce), see <c>GroupReduceFunction</c>. In the general case,
/// ReduceFunctions are considered faster, because they allow the system to use more efficient
/// execution strategies.</para>
///
/// <para>The basic syntax for using a grouped ReduceFunction is as follows:</para>
///
/// <code><![CDATA[
/// DataSet<X> input = ...;
///
...
public interface IReduceFunction<T> : IFunction
{
    ...
    /// <exception cref="Exception">This method may throw exceptions. Throwing an exception will
    /// cause the operation to fail and may trigger recovery.</exception>
